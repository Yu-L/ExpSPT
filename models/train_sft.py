...
def Customtokenizer(examples, pading=True, 
                    max_len=4096, integrity=False, truncate=True):
    """
...
def train(args):
    ...
    buffer_size = int(1e3)
    shuffle = True
    train_dataset = train_dataset.shuffle(buffer_size=buffer_size) if shuffle else train_dataset
    val_dataset = val_dataset.shuffle(buffer_size=buffer_size) if shuffle else val_dataset
