...
from transformers import BeamScorer, BeamSearchScorer, LogitsProcessorList, MinLengthLogitsProcessor, StoppingCriteriaList
import warnings
...
from models.utils import top_p, top_k, masked_mean, gumbel_sample, eval_decorator, gumbel_logits, gumbel_samples
...
class SPT(nn.Module):
    def __init__(
        ...
        *,
        ...
        # print("self.token_emb.weight.device: ", self.token_emb.weight.device)
        # fine tuning related
    ...
    def generate(
        ...
        **kwargs
        ...
        if num_beams:
            ...
            input_ids = prompt.cpu().long().repeat_interleave(num_beams, dim=0) # (num_beams * batch, ...)
            samples = self.beam_search(
                input_ids,
        ...
        for _ in wrapper_fn(range(sample_num_times)):
            ...
            # scores, sample = gumbel_sample(logits, temperature = temperature, dim=-1)
            scores, sample, probs = gumbel_logits(logits, temperature = temperature, dim=-1, prob=True)
    ...
    def beam_search(
        ...
        **kwargs
        ...
        while True:
            ...
            logits, embeds = self.forward(input_ids, return_logits_with_embedding = True, **kwargs)
            next_token_logits, embeds = logits[:, -1], embeds[:, -1]
            
            # next_token_logits = gumbel_logits(next_token_logits, temperature = 1.6)
            ...
            if beam_scorer.is_done or input_ids.shape[1]>=max_length:
                break
    ...
    def forward(
        ...
        # get token embedding
        #print(f"before emb.device: {x.shape}, self.device: {self.device}, x: {x}")
        x = self.token_emb(x)
        ...
        # print(f"logits.device: {logits.device}, labels.device: { labels.device}")
